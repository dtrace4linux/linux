...
err:
	p_dprintf("Pgrab_core: failed to read NT_PSTATUS\n");
	return (-1);
}
...
note_lwpstatus(struct ps_prochandle *P, size_t nbytes)
{
	...
	if ((lwp = lwpid2info(P, lps.pr_lwpid)) == NULL) {
		p_dprintf("Pgrab_core: failed to add NT_LWPSTATUS\n");
		return (-1);
	}
...
err:
	p_dprintf("Pgrab_core: failed to read NT_LWPSTATUS\n");
	return (-1);
}
...
note_psinfo(struct ps_prochandle *P, size_t nbytes)
{
	...
	p_dprintf("pr_fname = <%s>\n", P->psinfo.pr_fname);
	p_dprintf("pr_psargs = <%s>\n", P->psinfo.pr_psargs);
	p_dprintf("pr_wstat = 0x%x\n", P->psinfo.pr_wstat);

	return (0);

err:
	p_dprintf("Pgrab_core: failed to read NT_PSINFO\n");
	return (-1);
}
...
note_lwpsinfo(struct ps_prochandle *P, size_t nbytes)
{
	...
	if ((lwp = lwpid2info(P, lps.pr_lwpid)) == NULL) {
		p_dprintf("Pgrab_core: failed to add NT_LWPSINFO\n");
		return (-1);
	}
...
err:
	p_dprintf("Pgrab_core: failed to read NT_LWPSINFO\n");
	return (-1);
}
...
note_platform(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (nbytes != 0 && ((plat = malloc(nbytes + 1)) != NULL)) {
		if (read(P->asfd, plat, nbytes) != nbytes) {
			p_dprintf("Pgrab_core: failed to read NT_PLATFORM\n");
			free(plat);
			return (-1);
		}
		plat[nbytes - 1] = '\0';
...
note_utsname(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (read(P->asfd, utsp, ubytes) != ubytes) {
		p_dprintf("Pgrab_core: failed to read NT_UTSNAME\n");
		free(utsp);
		return (-1);
	}

	if (_libproc_debug) {
		p_dprintf("uts.sysname = \"%s\"\n", utsp->sysname);
		p_dprintf("uts.nodename = \"%s\"\n", utsp->nodename);
		p_dprintf("uts.release = \"%s\"\n", utsp->release);
		p_dprintf("uts.version = \"%s\"\n", utsp->version);
		p_dprintf("uts.machine = \"%s\"\n", utsp->machine);
	}
...
note_cred(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (read(P->asfd, pcrp, nbytes) != nbytes) {
		p_dprintf("Pgrab_core: failed to read NT_PRCRED\n");
		free(pcrp);
		return (-1);
	}

	if (pcrp->pr_ngroups > ngroups) {
		p_dprintf("pr_ngroups = %d; resetting to %d based on note size\n",
...
note_ldt(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (read(P->asfd, pldt, nbytes) != nbytes) {
		p_dprintf("Pgrab_core: failed to read NT_LDT\n");
		free(pldt);
		return (-1);
	}
...
note_priv(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (read(P->asfd, pprvp, nbytes) != nbytes) {
		p_dprintf("Pgrab_core: failed to read NT_PRPRIV\n");
		free(pprvp);
		return (-1);
	}
...
note_priv_info(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (read(P->asfd, ppii, nbytes) != nbytes ||
		...
		p_dprintf("Pgrab_core: failed to read NT_PRPRIVINFO\n");
		free(ppii);
		return (-1);
	}
...
note_zonename(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (nbytes != 0) {
		...
		if (read(P->asfd, zonename, nbytes) != nbytes) {
			p_dprintf("Pgrab_core: failed to read NT_ZONENAME\n");
			free(zonename);
			return (-1);
		}
		zonename[nbytes - 1] = '\0';
...
note_auxv(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (P->core->core_dmodel == PR_MODEL_ILP32) {
		...
		if (read(P->asfd, a32, nbytes) != nbytes) {
			p_dprintf("Pgrab_core: failed to read NT_AUXV\n");
			return (-1);
		}
...
note_xreg(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (read(P->asfd, xregs, xbytes) != xbytes) {
		p_dprintf("Pgrab_core: failed to read NT_PRXREG\n");
		free(xregs);
		return (-1);
	}
...
note_asrs(struct ps_prochandle *P, size_t nbytes)
{
	...
	if (read(P->asfd, asrs, sizeof (asrset_t)) != sizeof (asrset_t)) {
		p_dprintf("Pgrab_core: failed to read NT_ASRS\n");
		free(asrs);
		return (-1);
	}
...
{
	p_dprintf("skipping unsupported note type\n");
	return (0);
}
    ...
    GElf_Shdr *symtab, GElf_Shdr *strtab)
{
	...
	if (symtab->sh_addr == 0 ||
		...
		p_dprintf("fake_up_symtab: invalid section\n");
		return;
	}
	...
	if (P->status.pr_dmodel == PR_MODEL_ILP32) {
		...
		if (pread64(P->asfd, &b->data[off], b->shdr[1].sh_size,
			...
			p_dprintf("fake_up_symtab: pread of symtab[1] failed\n");
			free(b);
			return;
		}
		...
		if (pread64(P->asfd, &b->data[off], b->shdr[2].sh_size,
			...
			p_dprintf("fake_up_symtab: pread of symtab[2] failed\n");
			free(b);
			return;
		}
	...
	if ((scn = elf_getscn(fp->file_symtab.sym_elf, 1)) == NULL ||
		...
		p_dprintf("fake_up_symtab: failed to get section data at %p\n",
		    (void *)scn);
		goto err;
	}
...
core_elf_fdopen(elf_file_t *efp, GElf_Half type, int *perr)
{
	...
	if ((efp->e_hdr.e_shnum == 0 && efp->e_hdr.e_shoff != 0) ||
		...
		if (phdr.p_offset != 0 && phnum != efp->e_hdr.e_phnum) {
			p_dprintf("suspicious program header count %u %u\n",
			...
			if (phnum <= INT_MAX &&
				...
				p_dprintf("using new program header count\n");
			} else {
				p_dprintf("inconsistent program header count\n");
			}
		}
	}
...
core_iter_mapping(const rd_loadobj_t *rlp, struct ps_prochandle *P)
{
	...
	if ((fp = mp->map_file) == NULL &&
		...
		p_dprintf("failed to malloc mapping data\n");
		return (0); /* Abort */
	}
	fp->file_map = mp;
	...
	*fp->file_lo = *rlp;
	...
	if (fp->file_saddrs == NULL) {
		for (mp = fp->file_map + 1; mp < P->mappings + P->map_count &&
			...
			if (mp->map_file == NULL) {
				p_dprintf("core_iter_mapping %s: associating "
				    "segment at %p\n",
				    fp->file_pname,
				...
				p_dprintf("core_iter_mapping %s: segment at "
...
core_load_shdrs(struct ps_prochandle *P, elf_file_t *efp)
{
	...
	if (efp->e_hdr.e_shstrndx >= efp->e_hdr.e_shnum) {
		p_dprintf("corrupt shstrndx (%u) exceeds shnum (%u)\n",
	...
	if ((shdrs = malloc(efp->e_hdr.e_shnum * sizeof (GElf_Shdr))) == NULL) {
		p_dprintf("failed to malloc %u section headers: %s\n",
	...
	if ((buf = malloc(nbytes)) == NULL) {
		p_dprintf("failed to malloc %d bytes: %s\n", (int)nbytes,
		    strerror(errno));
		free(shdrs);
		goto out;
	}
	...
	if (pread64(efp->e_fd, buf, nbytes, efp->e_hdr.e_shoff) != nbytes) {
		p_dprintf("failed to read section headers at off %lld: %s\n",
	...
	if ((shstrtab = malloc(shstrtabsz + 1)) == NULL) {
		p_dprintf("failed to allocate %lu bytes for shstrtab\n",
		    (ulong_t)shstrtabsz);
		goto out;
	}
	...
	if (pread64(efp->e_fd, shstrtab, shstrtabsz,
		...
		p_dprintf("failed to read %lu bytes of shstrs at off %lld: %s\n",
	...
	for (i = 0; i < efp->e_hdr.e_shnum; i++) {
		...
		if (shp->sh_name >= shstrtabsz) {
			p_dprintf("skipping section [%d]: corrupt sh_name\n", i);
			continue;
		}

		if (shp->sh_link >= efp->e_hdr.e_shnum) {
			p_dprintf("skipping section [%d]: corrupt sh_link\n", i);
			continue;
		}

		p_dprintf("found section header %s (sh_addr 0x%llx)\n",
		    name, (u_longlong_t)shp->sh_addr);

		if (strcmp(name, ".SUNW_ctf") == 0) {
			if ((mp = Paddr2mptr(P, shp->sh_addr)) == NULL) {
				p_dprintf("no map at addr 0x%llx for %s [%d]\n",
			...
			if (mp->map_file == NULL ||
				...
				p_dprintf("no mapping file or duplicate buffer "
				    "for %s [%d]\n", name, i);
				continue;
			}
			...
			if ((buf = malloc(shp->sh_size)) == NULL ||
				...
				p_dprintf("skipping section %s [%d]: %s\n",
...
Pfgrab_core(int core_fd, const char *aout_path, int *perr)
{
	...
	if (lseek64(P->asfd, note_phdr.p_offset, SEEK_SET) == (off64_t)-1) {
		p_dprintf("Pgrab_core: failed to lseek to PT_NOTE data\n");
		*perr = G_STRANGE;
		goto err;
	}
	...
	for (nleft = note_phdr.p_filesz; nleft > 0; ) {
		...
		if (read(P->asfd, &nhdr, sizeof (nhdr)) != sizeof (nhdr)) {
			p_dprintf("Pgrab_core: failed to read ELF note header\n");
			*perr = G_NOTE;
			goto err;
		}
		...
		if (lseek64(P->asfd, namesz, SEEK_CUR) == (off64_t)-1) {
			p_dprintf("failed to seek past name and padding\n");
			*perr = G_STRANGE;
			goto err;
		}

		p_dprintf("Note hdr n_type=%u n_namesz=%u n_descsz=%u\n",
		...
		if (lseek64(P->asfd, off + nhdr.n_descsz,
		    SEEK_SET) == (off64_t)-1) {
			p_dprintf("Pgrab_core: failed to seek to next nhdr\n");
			*perr = G_STRANGE;
			goto err;
		}
	...
	if (nleft != 0) {
		p_dprintf("Pgrab_core: note section malformed\n");
		*perr = G_STRANGE;
		goto err;
	}
	...
	if ((pagesize = Pgetauxval(P, AT_PAGESZ)) == -1) {
		...
		p_dprintf("AT_PAGESZ missing; defaulting to %d\n", pagesize);
	}
	...
	if (intp_scn != NULL && (dp = elf_getdata(intp_scn, NULL)) != NULL &&
		...
		p_dprintf(".interp section is missing or could not be read; "
		    "defaulting to %s\n", interp);
	} else
		p_dprintf("detected statically linked executable\n");
	...
	if (base_addr != (uintptr_t)-1L) {
		...
		if (core_elf_open(&intf, interp, ET_DYN, NULL) == 0) {
			...
			if (dmp != NULL) {
				p_dprintf("renamed data at %p to %s\n",
	...
	if ((P->rap = rd_new(P, 0)) != NULL) {
		...
		p_dprintf("failed to initialize rtld_db agent\n");
