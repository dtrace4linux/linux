...
dt_modtext(char *p, GElf_Rela *rela, uint32_t *off)
{
	 ...
	 * linker invocation in which case we'd expect to see either
	 * a ret/restore pair or a nop; return success in that case.
	 */
	 ...
	 */
	assert(ip[0] == DT_OP_CALL);

	/*
	 * If the call is followed by a restore, it's a tail call so change
	 * the call to a ret. Otherwise we adjust the offset to land on what
	 * was once the delay slot of the call so we correctly get all the
	 * arguments.
	 */
	if (DT_IS_RESTORE(ip[1])) {
		ip[0] = DT_OP_RET;		/* ret */
...
process_obj(dtrace_hdl_t *dtp, const char *obj)
{
	...
	while ((scn_rel = elf_nextscn(elf, scn_rel)) != NULL) {
		...
		for (i = 0; i < shdr_rel.sh_size / shdr_rel.sh_entsize; i++) {
			...
			dt_dprintf("added instance %s:%s %x\n", p, s, off);

			if (dt_probe_add(dtp, prp, s, off) != 0)
				return (-1); /* errno is set for us */
