...
#pragma ident	"@(#)fasttrap_isa.c	1.13	04/11/17 SMI"
...
fasttrap_anarg(struct regs *rp, int function_entry, int argno)
{
	...
	if (curproc->p_model == DATAMODEL_LP64) {
		...
		value = dtrace_fulword(&stack[argno + shift]);
    ...
    fasttrap_tracepoint_t *tp, uintptr_t pc)
{
	 ...
	 */
	if (fasttrap_uread(p, &instr[0], first, pc) != 0)
		return (-1);
	if (len > first &&
	    fasttrap_uread(p, &instr[first], len - first, pc + first) != 0) {
