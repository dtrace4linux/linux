...
#pragma ident	"@(#)fbt.c	1.8	04/12/18 SMI"
...
#define	FBT_ENTENT_MAXSIZE	(12 * sizeof (uint32_t))
    ...
    int nargs)
{
	...
	if (FBT_IS_SAVE(save)) {
		uintptr_t ret = (uintptr_t)instr - 4;
		...
		*tinstr++ = FBT_RESTORE(FBT_REG_G0, FBT_REG_G0, FBT_REG_G0);
		...
		if (FBT_IS_BA(save)) {
			/*
			 * This is a special case:  we'll return directly
			 * to the destination of the branch, putting the
			 * save in the delay slot.
			 */
...
/*
 * We are patching control-transfer/restore couplets.  There are two variants
 * of couplet:
 *
 * (a)	jmpl		rs1 + (rs2 | offset), rd
 *	restore		rs1, rs2 | imm, rd
 *
 * (b)	call		displacement
 *	restore		rs1, rs2 | imm, rd
 *
 * If rd from the jmpl in (a) is something other than %g0 (a ret) or %o7 (a
 * call through a register), we fail.
 *
 * Note that rs1 and rs2 in the restore are potentially outputs and/or globals.
 * Because these registers cannot be relied upon across the call to
 * dtrace_probe(), we move rs1 into an unused local, ls0, and rs2 into an
 * unused local, ls1, and restructure the restore to be:
 *
 *	restore		ls0, ls1, rd
 *
 * Likewise, rs1 and rs2 in the jmpl of case (a) may be outputs and/or globals.
...
/*ARGSUSED*/
static int
fbt_canpatch_return(uint32_t *instr, int offset)
{
	int rd;
	 ...
	 * a warning and fail.
	 */
	cmn_err(CE_NOTE, "strange jmpl at %p", (void *)instr);
	return (0);
}

static int
fbt_canpatch_retl(uint32_t *instr, int offset)
{
	if (FBT_FMT1_OP(*instr) == FBT_OP_CALL ||
		...
		if (op != FBT_OP2 || rd != FBT_REG_O7) {
			cmn_err(CE_NOTE, "strange leaf jmpl/call "
			    "delay at %p", (void *)(instr + 1));
			return (0);
		}
	...
	if (offset == sizeof (uint32_t)) {
		 ...
		 */
		if (fbt_canpatch_retl(instr - 1, 0))
			return (1);
	}
...
fbt_patch_return(uint32_t *instr, uint32_t *funcbase, uint32_t *funclim,
    int offset, uint32_t id, fbt_trampoline_t *tramp)
{
	uint32_t *tinstr = (uint32_t *)tramp->fbtt_next;
    ...
    int offset, uint32_t id, fbt_trampoline_t *tramp)
{
	...
	if (offset == sizeof (uint32_t) && fbt_canpatch_retl(instr - 1, 0)) {
		*tinstr++ = *instr;
		annul = 1;
...
forreal:
	...
	for (i = 1; i < nsyms; i++) {
		...
		int (*canpatch)(uint32_t *, int);
		uint32_t (*patch)(uint32_t *, uint32_t *, uint32_t *, int,
		    uint32_t, fbt_trampoline_t *);
