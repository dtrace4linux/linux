# 1 "proc_arg.c"
# 1 "/home/fox/3rd/linux/dtrace/usr/src/lib/libproc/common//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "proc_arg.c"
# 10 "proc_arg.c"
...
# 18 "../../../linux/linux_types.h" 2
# 1 "../../../linux/zone.h" 1
# 19 "../../../linux/linux_types.h" 2
# 31 "../../../linux/linux_types.h"
extern int pread();
extern int pwrite();
# 82 "../../../linux/linux_types.h"
...
# 114 "../../../linux/linux_types.h" 2

typedef union {
        long double _q;
        uint32_t _l[4];
} upad128_t;
# 214 "../../../linux/linux_types.h"
...
# 228 "/usr/include/sys/stat.h"
extern int stat64 (__const char *__restrict __file,
     struct stat64 *__restrict __buf) __attribute__ ((__nothrow__)) __attribute__ ((__nonnull__ (1, 2)));
extern int fstat64 (int __fd, struct stat64 *__buf) __attribute__ ((__nothrow__)) __attribute__ ((__nonnull__ (2)));
# 261 "/usr/include/sys/stat.h"
extern int lstat (__const char *__restrict __file,
    struct stat *__restrict __buf) __attribute__ ((__nothrow__)) __attribute__ ((__nonnull__ (1, 2)));
# 274 "/usr/include/sys/stat.h"
...
# 1 "../../../linux/elf.h" 1 3 4
# 9 "../../../linux/elf.h" 3 4
# 1 "../../../linux/sys/elf_amd64.h" 1 3 4
# 30 "../../../linux/sys/elf_amd64.h" 3 4
#pragma ident "@(#)elf_amd64.h	1.4	05/06/08 SMI"
# 10 "../../../linux/elf.h" 2 3 4
...
# 68 "../../../linux/sys/signal.h"
typedef struct {
        uint32_t __sigbits[4];
} sigset32_t;
# 79 "../../../linux/sys/signal.h"
...
# 41 "libproc.h" 2
# 1 "../../libctf/common/libctf.h" 1
# 26 "../../libctf/common/libctf.h"
...
# 29 "../../libctf/common/libctf.h" 2
# 37 "../../libctf/common/libctf.h"
...
open_psinfo(const char *arg, int *perr)
{
 ...
 char *path = __builtin_alloca (strlen(arg) + 14);
 ...
 if ((fd = open64(path, 00)) >= 0) {
  if (fstat64(fd, &st) != 0 || !((((st.st_mode)) & 0170000) == (0100000)) ||
      strcmp(st.st_fstype, "proc") != 0) {
   (void) close(fd);
   fd = -1;
  }
 } else if ((*__errno_location ()) == 13 || (*__errno_location ()) == 1)
  *perr = 6;
...
open_core(const char *arg, int *perr)
{
 ...
 if ((fd = open64(arg, 00)) >= 0) {
  if (read(fd, &ehdr, sizeof (ehdr)) != sizeof (ehdr)) {
   (void) close(fd);
   fd = -1;
  } else if ((is_noelf = memcmp(&ehdr.e_ident[0], "\177ELF",
       4)) != 0 || ehdr.e_type != 4) {
    (void) close(fd);
    fd = -1;
    if (is_noelf == 0 &&
        ehdr.e_ident[5] != order)
     *perr = 15;
  }
 } else if ((*__errno_location ()) == 13 || (*__errno_location ()) == 1)
  *perr = 6;
...
{
 if ((oflag & (0x1 | 0x2)) == 0x1)
  return (1);

 if ((oflag & (0x1 | 0x2)) == 0x2)
  return (2);
 ...
 *perr = 0;
 ...
 if (lwps != ((void *)0) && (slash = strrchr(arg, '/')) != ((void *)0)) {
  ...
  *slash = '\0';
   ...
   *slash = '/';
   ...
   core = __builtin_alloca (strlen(arg) + 1);
   (void) strcpy(core, arg);
   if ((Pr = Pfgrab_core(fd, path == ((void *)0) ?
       dirname(core) : path, perr)) != ((void *)0)) {
    if (psp) {
 ...
 if ((oflag & 0x1) && (fd = open_psinfo(arg, perr)) != -1) {
 ...
 if ((oflag & 0x2) && (fd = open_core(arg, perr)) != -1) {
  core = __builtin_alloca (strlen(arg) + 1);
  (void) strcpy(core, arg);
  if ((Pr = Pfgrab_core(fd, path == ((void *)0) ? dirname(core) : path,
      perr)) != ((void *)0)) {
   if (psp) {
    (void) memcpy(psp, Ppsinfo(Pr),
        sizeof (psinfo_t));
...
{
 return (proc_grab_common(arg, path, oflag, gflag, perr, lwps, ((void *)0)));
}

struct ps_prochandle *
proc_arg_grab(const char *arg, int oflag, int gflag, int *perr)
{
 return (proc_grab_common(arg, ((void *)0), oflag, gflag, perr, ((void *)0), ((void *)0)));
}
...
proc_arg_psinfo(const char *arg, int oflag, psinfo_t *psp, int *perr)
{
 ...
 if (proc_grab_common(arg, ((void *)0), oflag, 0, perr, ((void *)0), psp) == ((void *)0))
  return (-1);
 else
  return (psp->pr_pid);
}
    ...
    const char **lwps)
{
 ...
 if (proc_grab_common(arg, ((void *)0), oflag, 0, perr, lwps, psp) == ((void *)0))
  return (-1);
 else
  return (psp->pr_pid);
}
